...
char mqtt_temp_topic[50] = "home/wohnzimmer/lixie/temp";
...
const unsigned int tempInterval = 60000;
...
void setup()
{
    ...
    if (SPIFFS.exists("/config.json")) {
      ...
      if (configFile) {
        ...
        if (json.success()) {
          ...
          strcpy(mqtt_temp_topic, json["mqtt_topic"]);
          sprintf(mqtt_temp_topic, "%s%s", mqtt_topic, "/temp");
  ...
  sprintf(mqtt_cmd_topic, "%s%s", mqtt_topic, "/cmd");
  strcpy(mqtt_temp_topic, custom_mqtt_topic.getValue());
  sprintf(mqtt_temp_topic, "%s%s", mqtt_topic, "/temp");
...
void loop()
{
  ...
  if((millis() - lastTempUpdate) > tempInterval)
  {
    ...
    client.publish(mqtt_temp_topic, bmeJson, true, 0);
  }
